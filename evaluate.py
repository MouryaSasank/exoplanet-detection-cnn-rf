...
This module produces all evaluation outputs:
  - Classification metrics (Precision, Recall, F1, PR-AUC)
...
def compute_metrics(y_true, y_pred, y_prob):
    """
    Compute precision, recall, F1, and Precision-Recall AUC.

    We use PR-AUC instead of ROC-AUC because it is more informative
    for severely imbalanced datasets like ours.
...
def plot_feature_importance(rf_model, save_path, top_n=20):
    ...
    ax.set_title(f'Top {top_n} Feature Importances (Hybrid RF)', fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
def generate_comparison_table(baseline_metrics, hybrid_metrics, save_path):
    ...
    comparison = pd.DataFrame({
        'Metric': ['Precision', 'Recall', 'F1-Score', 'PR-AUC', 'Accuracy'],
        'Baseline RF': [
...
def generate_html_report(baseline_metrics, hybrid_metrics, output_dir, cv_results_path=None):
    ...
    if cv_results_path and os.path.isfile(cv_results_path):
        ...
        cv_prec_mean  = f"{cv_means['Precision']:.4f}"
        cv_prec_std   = f"{cv_stds['Precision']:.4f}"
        cv_rec_mean   = f"{cv_means['Recall']:.4f}"
        cv_rec_std    = f"{cv_stds['Recall']:.4f}"
        cv_f1_mean    = f"{cv_means['F1']:.4f}"
        cv_f1_std     = f"{cv_stds['F1']:.4f}"
        cv_prauc_mean = f"{cv_means['PR_AUC']:.4f}"
        cv_prauc_std  = f"{cv_stds['PR_AUC']:.4f}"
        cv_fold_rows  = ''
        for _, row in cv_df.iterrows():
            cv_fold_rows += (
                ...
                f'<td class="highlight">{row["PR_AUC"]:.4f}</td></tr>'
            )
        cv_section = (
            ...
            '<thead><tr><th>Fold</th><th>Precision</th><th>Recall</th><th>F1-Score</th><th>PR-AUC</th></tr></thead>'
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        /* ═══════════════════════════════════════════
           METRIC STAT CARDS (Hero Numbers)
           ═══════════════════════════════════════════ */
        .stats-grid {{
            display: grid;
        ...
        .stat-card:hover {{
            ...
            box-shadow: 0 20px 60px rgba(41, 151, 255, 0.12),
        ...
        .stat-value.purple {{ color: #bf5af2; }}
        .stat-value.orange {{ color: #ff9f0a; }}
        .stat-value.pink {{ color: #ff375f; }}
        ...
        .report-card.baseline:hover {{
            border-color: rgba(255, 55, 95, 0.3);
        }}

        .report-card.hybrid:hover {{
...
<body>
    ...
    <!-- ═══ HERO ═══ -->
    <header class="hero">
        <div class="hero-eyebrow" id="heroEyebrow">Hybrid Deep Feature Extraction</div>
        <h1 id="heroTitle">Exoplanet Detection<br>Results Report</h1>
        <p id="heroSub">A 1D CNN learns transit patterns from Kepler light curves, then a Random Forest classifies the deep features — outperforming raw-flux baselines.</p>
    </header>
        ...
        <section class="section" id="metrics">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card reveal delay-5">
                    <div class="stat-value pink" data-counter="{hybrid_metrics['accuracy']:.4f}">{hybrid_metrics['accuracy']:.4f}</div>
            ...
            <div class="table-card reveal delay-2">
                <table>
                    <thead>
                        <tr>
                            <th>Metric</th>
                            <th>Baseline RF (Raw Flux)</th>
                            <th>Hybrid CNN+RF (Deep Features)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Precision</td>
                            <td class="dim">{baseline_metrics['precision']:.4f}</td>
                            <td class="highlight">{hybrid_metrics['precision']:.4f}</td>
                        </tr>
                        <tr>
                            <td>Recall</td>
                            <td class="dim">{baseline_metrics['recall']:.4f}</td>
                            <td class="highlight">{hybrid_metrics['recall']:.4f}</td>
                        </tr>
                        <tr>
                            <td>F1-Score</td>
                            <td class="dim">{baseline_metrics['f1']:.4f}</td>
                            <td class="highlight">{hybrid_metrics['f1']:.4f}</td>
                        </tr>
                        <tr>
                            <td>PR-AUC</td>
                            <td class="dim">{baseline_metrics['pr_auc']:.4f}</td>
                            <td class="highlight">{hybrid_metrics['pr_auc']:.4f}</td>
                        </tr>
                        <tr>
                            <td>Accuracy</td>
                            <td class="dim">{baseline_metrics['accuracy']:.4f}</td>
                            <td class="highlight">{hybrid_metrics['accuracy']:.4f}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <div class="footer">
        ...
        <p style="margin-top:0.5rem;">Built with TensorFlow &bull; Scikit-learn &bull; Kepler Space Telescope Data</p>
    </div>
    ...
    <script>
        ...
        window.addEventListener('load', () => {{
            ...
            setTimeout(() => {{
                eyebrow.style.transition = 'opacity 0.7s cubic-bezier(0.4,0,0.2,1), transform 0.7s cubic-bezier(0.4,0,0.2,1)';
            ...
            setTimeout(() => {{
                sub.style.transition = 'opacity 0.8s cubic-bezier(0.4,0,0.2,1), transform 0.8s cubic-bezier(0.4,0,0.2,1)';
                sub.style.opacity = '1';
        ...
        const counterObserver = new IntersectionObserver((entries) => {{
            entries.forEach(entry => {{
                if (entry.isIntersecting) {{
                    // Animate counter up every time it enters view
                    ...
                    function animate(currentTime) {{
                        ...
                        entry.target.textContent = current.toFixed(4);
                        if (progress < 1) requestAnimationFrame(animate);
                    }}

                    requestAnimationFrame(animate);
                }} else {{
                    // Reset to 0 when scrolled away so it re-animates on return
                    entry.target.textContent = '0.0000';
                }}
            }});
        }}, {{ threshold: 0.5 }});
        ...
        // ── Parallax star field on scroll ──
        window.addEventListener('scroll', () => {{
            const scrolled = window.pageYOffset;
            const field = document.getElementById('starfield');
            field.style.transform = 'translateY(' + (scrolled * 0.15) + 'px)';
        }}, {{ passive: true }});
    </script>
</body>
</html>"""
